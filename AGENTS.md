...
|---|---|---|
| Turn/round state machine | `TestDebateStateMachineReachesConsensus`, `TestTurnOrderSurvivesSubSecondJoinTimes`, `TestStuckModerationResumesWithoutRestart`, `TestModerationAttemptBookkeepingHoldsTheSinglePass`, `TestModerationRetryBacksOff`, `TestLateResumeDoesNotRemoderateAConcludedDebate`, `TestScriptedHybridDemoCompletesWithoutLLMKeys` | enforced by `make check` |
| HTTP trust boundary: auth and abuse limits | `TestAuthenticateRejectsUnknownKey`, `TestRegisterRateLimitRejectsBurstFromOneClient`, `TestCreateDebateRateLimitIsPerAgentKey`, `TestStreamLimitReleasesSlotOnDisconnect`, `TestWorstCaseExportFitsTheDeclaredBudget`, `TestExportSharesTheAddressStreamBudget`, `TestExportIsRefusedWhenTheProcessCeilingIsFull`, `TestOneAddressCannotHoldTheWholeExportCeiling`, `TestExportCeilingRefusalIsObservable`, `TestExportWriteCannotPinTheCeilingForever`, `TestExportSnapshotLeavesTheQueueWhenTheCallerIsGone`, `TestShippedDefaultsAreEnforcedByTheProductionHandler`, `TestMCPToolSchemasKeepCredentialsOutOfArguments`, `TestMCPInvalidBearerCannotRegisterReplacement`, `TestCredentialBootstrapInstructionsKeepSecretIssuanceInCleanTasks` | enforced by `make check` |
...
| REST/MCP core-state parity and the exported artifact | `TestExportedDebateIsAValidGoldenTrace`, `TestExportBytesAreExactlyTheSharedProducerOutput`, `TestExportWithholdsWhatTheDebateViewWithholds`, `TestExportSnapshotDoesNotMixTwoMoments`, `TestExportSnapshotFailsInsteadOfDroppingVotes`, `TestEveryGoldenTraceConformsToSpec`, `TestConformanceRejectsEachViolation`, `TestEverySpecRuleIsEnforcedAndDocumented`, `TestSpecStatesTheLimitsItIsCheckedAgainst`, `TestCheckRecordsEnforcesTheWholeStream`, `TestExportedArtifactConformsToSpec`, `TestHybridRecordsEverySkippedModerationResult`, `TestUnavailableModeratorNoticeIsRecordedOncePerRound`, `TestARecordedDegradationBindsItsRound`, `TestSpecPublishesTheDegradationNoticesTheServiceEmits`, `TestMCPWhoAmIReturnsStableIdentity`; issues #35 and #33 | REST export and protocol conformance enforced by `make check`; MCP has no export tool (#35) and execution metadata is undelivered (#33) — both remain tracked debt |
| Moderator spend boundary and credentials | `TestModeratorSpendCeilingDegradesToDeterministicVerdict`, `TestModeratorSpendCeilingKeepsConsensusFoundBeforeExhaustion`, `TestModeratorSpendCeilingSurvivesRestart`, `TestModeratorSpendCeilingChargesUnreportedUsageAsEstimate`, `TestModeratorSpendCeilingChargesFailedCalls`, `TestModeratorSpendCeilingDoesNotChargeUnbilledCalls`, `TestModeratorSpendCeilingRecordsOneNoticePerRoundAcrossRetries`, `TestModerationRetryWaitsForTheModerationInFlight`, `TestPaidRetriesAreCappedAndFreeOnesAreNot`, `TestStuckModerationStopsPayingWhenTheChargeIsLost`, `TestResumedConclusionKeepsTheDegradationAttribution`, `TestReservedModeratorNameIsRefusedAtStartup`, `TestUsageTravelsWithModerationErrors`, `TestUnavailableModeratorIsNotReportedAsBudgetDegradation`, `TestFixedPromptBytesFitTheBudgetReserve`, `TestAnthropicCallToolReportsUsage`, `TestShippedModeratorBudgetIsEnforcedByTheProductionService`, `TestComposeDemoDefaultsAreCredentialFree`, `TestDockerBuildContextExcludesDotEnvSecrets`, `TestRotationKeepsAgentIdentityStable`, `TestRevokedCredentialStopsAuthenticating`, `TestLastActiveCredentialCannotBeRevoked`, `TestCredentialOperationsRejectAnotherAgentsCredential`, `TestRevocationTombstonesTheRollbackShadow`, `TestTombstonedShadowSurvivesThePreviousBinaryMigration` | spend and credential rotation enforced by `make check`; web key storage (#20) remains tracked debt |
