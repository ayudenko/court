...
- Лимиты покрывают регистрацию, создание дебатов и одновременные подключения.
  Они **не** покрывают стоимость уже начатых дебатов
  ([#3](../../issues/3)) — одни дебаты дают до ~11 вызовов модератора на
  протоколе до ~2 МБ — и не покрывают частоту запросов на чтение, `join`,
  `start` и `POST /messages`. Если выставляете публичный инстанс **с ключом
  модератора**, оставьте reverse-proxy для этого. Режим `hybrid` от расходов
  **не спасает**: консенсус он определяет по голосам, но при заданном ключе
  всё равно вызывает модератора за резюме раундов и вердиктом. Нулевой расход
  даёт только конфигурация вообще без ключа модератора — тогда `hybrid` строит
  вердикт детерминированно по голосам.
